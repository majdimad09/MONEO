        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-2">
              <p className="text-sm font-bold leading-tight" style={{ color: colors.textPrimary }}>{challenge.name}</p>
              <div className="flex items-center gap-1.5 flex-shrink-0">
                <div className="w-1.5 h-1.5 rounded-full" style={{ background: statusColor }} />
                <span className="text-[10px] font-bold capitalize" style={{ color: statusColor }}>{status}</span>
              </div>
            </div>
            <p className="text-[10px] mt-0.5" style={{ color: colors.textMuted }}>{challengeTypeLabel(challenge.type)}</p>
            {joined && myPct !== null && (
              <div className="mt-2.5">
                <div className="flex justify-between text-[10px] mb-1">
                  <span style={{ color: colors.textMuted }}>You: {myPct}%</span>
                  <span style={{ color: colors.textSecondary }}>Community: {avg}%</span>
                </div>
                <div className="h-1.5 rounded-full overflow-hidden" style={{ background: colors.bgSecondary }}>
                  <div
                    ...
                    style={{
                      width: `${myPct}%`,
                      background: myPct >= 80 ? '#34d399' : myPct >= 40 ? '#fbbf24' : '#818cf8',
                    }}
                  />
                </div>
              </div>
          ...
          {expanded
            ? <ChevronUp size={14} className="flex-shrink-0 mt-1" style={{ color: colors.textMuted }} />
            : <ChevronDown size={14} className="flex-shrink-0 mt-1" style={{ color: colors.textMuted }} />}
        </div>
      </button>

      {expanded && (
        <div className="px-4 pb-4 space-y-3" style={{ borderTop: `1px solid ${colors.divider}` }}>
          <p className="text-xs pt-2 leading-relaxed" style={{ color: colors.textMuted }}>{challenge.description}</p>
          ...
          <div className="flex items-center gap-4 text-[11px]">
            ...
            <span style={{ color: colors.textSecondary }}>{total} days · {elapsed} elapsed</span>
          </div>
          ...
          {joined && challenge.type === 'custom' && status === 'active' && (
            <div>
              <p className="text-[10px] mb-2 font-semibold uppercase tracking-widest" style={{ color: colors.textMuted }}>Mark Progress</p>
              <div className="flex items-center gap-3">
                <button
                  onClick={() => onManual(false)}
                  className="w-8 h-8 rounded-lg flex items-center justify-center cursor-pointer font-bold"
                  style={{ background: colors.bgSecondary, border: `1px solid ${colors.border}`, color: colors.textMuted }}
                >
                  −
                </button>
                <div className="flex-1 text-center">
                  <p className="text-lg font-bold" style={{ color: colors.textPrimary }}>{liveParticipant.manualDays ?? 0} / {total}</p>
                  <p className="text-[10px]" style={{ color: colors.textSecondary }}>days marked</p>
                </div>
                <button
                  onClick={() => onManual(true)}
                  className="w-8 h-8 rounded-lg flex items-center justify-center cursor-pointer font-bold"
                  style={{ background: colors.brandSoft, border: `1px solid rgba(129,140,248,0.25)`, color: colors.brand }}
                >
                  +
                </button>
              </div>
          ...
          {challenge.participants.length > 0 && (
            <div>
              <p className="text-[10px] font-bold uppercase tracking-widest mb-2" style={{ color: colors.textSecondary }}>Participants</p>
              <div className="space-y-1.5">
                {challenge.participants.slice(0, 5).map(p => (
                  <div key={p.userId} className="flex items-center justify-between">
                    <span className="text-xs" style={{ color: colors.textMuted }}>{p.displayName}</span>
                    <div className="flex items-center gap-2">
          ...
          {!joined && status !== 'completed' && (
            <button
              onClick={onJoin}
              className="w-full py-2.5 rounded-xl text-sm font-bold btn-primary cursor-pointer flex items-center justify-center gap-2"
            >
...
const CreateChallengePanel: React.FC<{
  ...
  return (
    <div
      ...
      style={{ background: colors.bgCard, border: '1px solid rgba(139,92,246,0.25)' }}
    >
      <div className="flex items-center justify-between">
        <p className="text-sm font-bold" style={{ color: colors.textPrimary }}>New Challenge</p>
        <button onClick={p.onCancel} className="cursor-pointer" style={{ color: colors.textMuted }}><X size={16} /></button>
      </div>

      <div>
        <label className="text-[10px] font-bold uppercase tracking-widest block mb-2" style={{ color: colors.textMuted }}>Challenge Type</label>
        ...
        <p className="text-[11px] mt-2" style={{ color: colors.textSecondary }}>{challengeTypeDescription(p.chType)}</p>
      </div>

      <div>
        <label className="text-[10px] font-bold uppercase tracking-widest block mb-1.5" style={{ color: colors.textMuted }}>Challenge Name</label>
      ...
      {p.chType === 'score_boost' && (
        <div>
          <label className="text-[10px] font-bold uppercase tracking-widest block mb-1.5" style={{ color: colors.textMuted }}>Target Score</label>
      ...
      {p.chType === 'category_cut' && (
        <div className="grid grid-cols-2 gap-2">
          ...
          <div>
            <label className="text-[10px] font-bold uppercase tracking-widest block mb-1.5" style={{ color: colors.textMuted }}>Reduce by %</label>
      ...
      {p.chType === 'custom' && (
        <div>
          <label className="text-[10px] font-bold uppercase tracking-widest block mb-1.5" style={{ color: colors.textMuted }}>What's the goal?</label>
      ...
      <div className="grid grid-cols-2 gap-2">
        <div>
          <label className="text-[10px] font-bold uppercase tracking-widest block mb-1.5" style={{ color: colors.textMuted }}>Start</label>
          <input className={inputBase} type="date" value={p.chStart} onChange={e => p.setChStart(e.target.value)} />
        </div>
        <div>
          <label className="text-[10px] font-bold uppercase tracking-widest block mb-1.5" style={{ color: colors.textMuted }}>End</label>
